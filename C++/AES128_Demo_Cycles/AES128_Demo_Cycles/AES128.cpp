...
void run_AES()
{
	...
	//t = clock() - t;
	//double CycleCount = (GetCPUCount(lo, hi) - t2) / num;
	//double s = double(CLOCKS_PER_SEC);

	//cout << "\nCycles: " << CycleCount;
	//cout << "\n";
	//cout << "Time: " << ((float)t / CLOCKS_PER_SEC) / num;
	
	auto stop = std::chrono::high_resolution_clock::now();
	std::chrono::duration<double>elapsed = (stop - start)/num;
